<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-4">
      <h2 class="text-lg font-bold mb-4">文章标签</h2>
    ...
    <!-- 文章列表 -->
    <div class="bg-white rounded-lg shadow p-4">
      <h2 class="text-lg font-bold mb-4">{{ selectedTag ? selectedTag.tagName : '全部' }}文章</h2>
      ...
      <template v-else>
        ...
        <!-- 无数据状态 -->
        <div v-else class="text-center py-10 text-gray-500">该标签下暂无文章</div>
      </template>
...
import { getNoPaginationTagListService } from '@/api/tag'
...
const initTagId = route.query.tagId ? parseInt(route.query.tagId) : null
...
onMounted(async () => {
  ...
  // 如果有初始标签ID，设置选中的标签
  if (initTagId) {
    const tag = tags.value.find((t) => t.id === initTagId)
    if (tag) {
      selectedTag.value = tag
    }
  }
